...
# --- ELEMENTY SIECI ---
# Slack i Gen 2 są matematycznie niezbędne do działania sieci, 
# ale ich wizualne etykiety zostały usunięte z wykresu.
pp.create_ext_grid(net, bus=b1, vm_pu=1.06, name="Slack")
pp.create_gen(net, bus=b2, p_mw=40, vm_pu=1.01, name="Gen 2")

# UWAGA: Brak Generatora 5! 
# Został usunięty, aby pokazać surowy stan awaryjny (przeciążenie 131.3%)
...
# --- LINIE ---
line_data = [
    (b1, b2, 0.02, 0.06, 50, 1.5), # Index 0 (Linia krytyczna)
...
# ==============================================================================
# 2. OBLICZENIA I SYMULACJA AWARII (N-1)
# ==============================================================================
# Wyłączamy Linię nr 0, aby zasymulować awarię
LINE_OUTAGE = 0 
...
# ZWIĘKSZONY ZAKRES (vmax=140), by pomieścić przeciążenie 131.3% w skali barw
...
# C. DRUGA LEGENDA (Obciążenia linii - zaktualizowana do 140%)
fig.add_trace(go.Scatter(
    x=[None], y=[None], mode='markers',
    marker=dict(
        size=0, color=[0, 140], colorscale='turbo', cmin=0, cmax=140,
...
fig.update_layout(
    title=dict(
        text=f"<b>Power Flow Analysis - Before adding generator (Disconnected line number {LINE_OUTAGE})</b>", 
        font=dict(size=24)
    ),
