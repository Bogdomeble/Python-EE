...
print(f"Theoretical resonance frequency {f_res_theoretical:.2f} Hz\n")

# Pyspice circuit definition

circuit = Circuit('Series RLC Circuit')

# 1V AC power source with no stated frequency
...
circuit.L(1, 'node1', 'node2', L_val)
circuit.C(1, 'node2', circuit.gnd, C_val)
...
max_current_index = np.argmax(current_magnitude)
f_res_simulated = frequencies[max_current_index]

print(f"Simulated resonance frequncy: {float(f_res_simulated):.2f} Hz\n")
...
plt.title('Series RLC circuit - frequency response')
